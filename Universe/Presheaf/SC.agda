...
module Fill (fillable : ∀ n → Fillable P n) where
  ...
  module Continue {asc : ASC (suc k)} (sc : SC asc) where
    ...
    topes-compat : ∀ l m {s : CSet (suc k) l} {t : CSet (suc k) m} (s⊂t : s ⊂ t) → topes l s P.≃ P.proj (embed s⊂t) (topes m t)
    
    topes 0 s = sc .for (pser (_∈ asc) (l≡0 s Eq.refl) (empty∈asc asc))
    topes 1 s = sc .for (hasAllPoints asc s)
    topes (suc (suc l)) s =
      if has asc (_ , s)
      then sc .for
      else λ _ → fill (fillable l) ⟨$⟩ record
    ...
    topes-compat 0 0 s⊂t =
      topes 0 _ ≡⟨⟩
      sc .for _ ≡⟨ sc .for =$= T-irrel ⟩
      sc .for (Has-⊆ asc s⊂t _) ≈⟨ sc .compat _ s⊂t ⟩
      P.proj (embed s⊂t) (sc .for _) ≡⟨⟩
      P.proj (embed s⊂t) (topes 0 _) ∎
      where open Relation.Reasoning (P._≃_)
            open Equiv (refl (P.Space _)) (trig (P.Space _))

    topes-compat (suc _) 0 s⊂t = ⊂⇒≤ s⊂t |> λ ()

    topes-compat 0 1 s⊂t =
      topes 0 _ ≡⟨⟩
      sc .for _ ≡⟨ sc .for =$= T-irrel ⟩
      sc .for (Has-⊆ asc s⊂t _) ≈⟨ sc .compat _ s⊂t ⟩
      P.proj (embed s⊂t) (sc .for _) ≡⟨⟩
      P.proj (embed s⊂t) (topes 1 _) ∎
      where open Relation.Reasoning (P._≃_)
            open Equiv (refl (P.Space _)) (trig (P.Space _))
            
    topes-compat 1 1 s⊂t =
      topes 1 _ ≡⟨⟩
      sc .for _ ≡⟨ sc .for =$= T-irrel ⟩
      sc .for (Has-⊆ asc s⊂t _) ≈⟨ sc .compat _ s⊂t ⟩
      P.proj (embed s⊂t) (sc .for _) ≡⟨⟩
    ...
    topes-compat 0 (suc (suc l)) s⊂t = {!!}    
    topes-compat 1 (suc (suc l)) s⊂t = {!!}
    topes-compat (suc (suc l)) (suc (suc m)) s⊂t = {!!}

--   continue : {asc : ASC (suc k)} → SC asc → SC (all (suc k))
--   continue {asc = asc} sc .for {0} {s} _ = sc .for (pser (_∈ asc) (l≡0 s Eq.refl) (empty∈asc asc))
--   continue {asc = asc} sc .for {1} {s} _ = sc .for (hasAllPoints asc s)
--   continue {asc = asc} sc .for {suc l @ (suc m)} {s} _ =
--     if has asc (_ , s)
--     then sc .for
--     else
--       let cycl = record { face = λ i → continue sc .for {l} tt ; compatible = {!!} }
--       in λ _ → fill (fillable m) ⟨$⟩ cycl
--   continue sc .compat = {!!}
